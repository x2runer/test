-- TRUEDEX V2 - Full UI Library (Fixed Version 2026)
-- Tab hiển thị đúng + Search hoạt động tốt
...
function TRUEDEXV2:CreateWindow(config)
    ...
    local function addCommonMethods(Target, ParentScroll)
        ...
        function Target:AddColorPicker(text, default, callback, flag)
            ...
            track(preview.MouseButton1Click:Connect(function()
                ...
                local svConn1 = track(svBox.InputBegan:Connect(function(i)
                ...
                local hueConn1 = track(hueBar.InputBegan:Connect(function(i)
                    if i.UserInputType == Enum.UserInputType.MouseButton1 then
                        draggingHue = true
                ...
                local globalDrag = track(UserInputService.InputChanged:Connect(function(i)
                    if draggingSV then
                        local relX = math.clamp((i.Position.X - svBox.AbsolutePosition.X) / svBox.AbsoluteSize.X, 0, 1)
                        local relY = math.clamp((i.Position.Y - svBox.AbsolutePosition.Y) / svBox.AbsoluteSize.Y, 0, 1)
                        sat, val = relX, 1 - relY
                ...
                local globalEnd = track(UserInputService.InputEnded:Connect(function(i)
                    if i.UserInputType == Enum.UserInputType.MouseButton1 then
                        draggingSV = false
                        draggingHue = false
                    end
    ...
    function Window:CreateTab(name, iconId)
        ...
        icon.ImageColor3 = TRUEDEXV2.THEME.TEXT
        icon.Parent = b
        ...
        titleLabel.TextColor3 = TRUEDEXV2.THEME.TEXT_DIM
        titleLabel.Font = Enum.Font.GothamBold
